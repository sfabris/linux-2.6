...
static struct dvfm_md_opt pxa310_op_array[] = {
	/* 60MHz -- ring oscillator */
	{
		.vcc_core = 1000,
		.vcc_sram = 1100,
		...
		.sflfs = 60,
		.hss = 60,
		.dmcfs = 60, /* will be 60MHZ for PXA310 A2 and PXA935/PXA940 */
		.df_clk = 15,
		.empi_clk = 15,
		.power_mode = POWER_MODE_D0CS,
		.flag = OP_FLAG_FACTORY,
...
static struct proc_op_array proc_op_arrays[] = {
	...
	{0x6890, "PXA310", ARRAY_AND_SIZE(pxa310_op_array)},
...
static int op_init(void *driver_data, struct info_head *op_table)
{
	...
	for (i = 0, index = 0; i < proc->nr_op; i++) {
		...
		if (proc->op_array[i].power_mode == POWER_MODE_D0CS) {
			if ((info->cpuid == 0x6892) || (info->cpuid == 0x6893)
				|| ((info->cpuid & 0xFFF0) == 0x6930)) {
				set_dmc60(info, 1);
				proc->op_array[i].dmcfs = 60;
			} else
				proc->op_array[i].dmcfs = 30;
		} else if (proc->op_array[i].power_mode == POWER_MODE_D0) {
                        /* PXA310 Ax steppings, dmcfs is 260MHz. 6893--B1
			 * only PXA320 and PXA310 B1 support ddr@156M. */
                        if (((info->cpuid & 0xFFF0) == 0x6820)
                                || (info->cpuid == 0x6893)) {
                                if (((ACCR >> ACCR_DMCFS_OFFSET) & 0x03) == 1)
                                        proc->op_array[i].dmcfs = 312;
                                else
                                        proc->op_array[i].dmcfs = 260;
                        } else {
                                proc->op_array[i].dmcfs = 260;
                        }
		}
...
static void pxa3xx_poweri2c_init(struct pxa3xx_dvfm_info *info)
{
	...
	if ((info->flags & PXA3xx_USE_POWER_I2C) &&
		 ...
		 * PXA310 B1(cpuid 0x6893) requires 1450mv in 624MHz/806MHz
		 */
		switch (info->cpuid) {
		...
		case 0x6838:		/* PXA930 B2 */
			...
			break;
		case 0x6893:		/* PXA310 B1 */
			avcr |= (0x01d << 24) | (0x1d << 16) | (0x0f << 8) | 0xb;
