...
static int pxa_create_dma( struct fb_info *info, int buffer_index)
{    
	struct overlayfb_info *fbi = (struct overlayfb_info*) info;
...
static int overlay2fb_map_YUV_memory( struct fb_info *info)
{
	...
	fbi->map_cpu = (unsigned long)ov2_dma_alloc_writecombine((struct fb_info *)(fbi->basefb), fbi->map_size,
					       &fbi->map_dma, GFP_KERNEL );
	if (!fbi->map_cpu) {
...
static int overlay2fb_YUV420_workaround(struct fb_info *info)
{
	...
	map_cpu = (u32)ov2_dma_alloc_writecombine((struct fb_info *)(fbi->basefb), PAGE_SIZE, &map_dma, GFP_KERNEL );
