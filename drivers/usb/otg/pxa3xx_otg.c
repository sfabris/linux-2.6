...
extern struct otg_pmic_ops *init_otg_pmic_ops(void);
...
static void otg_b_idle(struct pxa_otg *pxa_otg)
{
	if (pxa_otg->otg.state == OTG_STATE_B_IDLE)
		return;
	/* Disable usb pump */
	pxa_otg->pmic_ops->otg_set_vbus(VBUS_LOW);

	otg_init_handle(pxa_otg);
	pxa_otg->otg.default_a = OTG_B_DEVICE;
	pxa_otg->otg.state = OTG_STATE_B_IDLE;
	pxa_otg->pmic_ops->otg_set_vbus_ic(pxa_otg->otg.default_a);
...
int otg_init(struct pxa_otg *pxa_otg)
{
	...
	pxa_otg->pmic_ops->otg_vbus_init();
...
static void pxa3xx_update_otg(struct pxa_otg *pxa_otg)
{
	...
	int timer_id;
	struct otg_xceiv_ops *xceiv_ops = pxa_otg->xceiv_ops;
	struct otg_pmic_ops *pmic_ops = pxa_otg->pmic_ops;
	...
	if ((state != pxa_otg->otg.state)) {
		...
		case OTG_STATE_A_WAIT_VRISE:
			/* Enable USB PUMP */
			pmic_ops->otg_set_vbus(VBUS_HIGH);
			pmic_ops->otg_set_vbus_ic(pxa_otg->otg.default_a);
			if (xceiv_ops->otgx_check_vbus())
				pxa_otg_vbus_handle(USBOTG_VBUS_VALID);
			break;
		case OTG_STATE_A_WAIT_BCON:
			...
			if (pxa_otg->otg_ctrl->a_wait_bcon_tmr == -1) {
				timer_id = otg_register_timer(pxa_otg,
					T_A_WAIT_BCON, otg_timer_await_bcon);
				if (timer_id < 0)
		...
		case OTG_STATE_A_WAIT_VFALL:
			...
			break;
		case OTG_STATE_A_VBUS_ERR:
			pmic_ops->otg_set_vbus(VBUS_LOW);
...
static int pxa_otg_probe(struct platform_device *pdev)
{
	...
	pxa_otg->pmic_ops = init_otg_pmic_ops();
#ifdef CONFIG_USB_PXA9XX_U2O 
	if (cpu_is_pxa935()) {
